...
and **em-dashes used as an all-purpose connector** (thin them by the density rule in Layer 3, not by a blanket ban).
...
- **Em-dashes: judge by density against the author's own baseline; do not ban them.** Check the author's
  earlier writing or the venue's house style first. If they use dashes, keep them at that rate and strip
  only the excess -- the ones standing in for a comma, colon, or full stop that the sentence would carry
  better. If their prior work has none, remove them. Never trade a dash for a worse sentence. This is the
  same rule `voice-layer` applies (it objects to dash density, not to the dash), so the two skills do not
...
- **NIH (R01).** The **Specific Aims page is the whole proposal in one page**, and is the most-read,
  ...
  Then the **Research Strategy** sections -- Significance, Innovation, Approach. Those are *document
  sections*; they are not the score sheet, and conflating the two is the most common way a proposal gets
  edited to the wrong target.

**Check the review criteria against the application's due date and its own funding announcement before
editing to them.** For most research grant applications due on or after 2025-01-25, NIH scores three
factors that do not map one-to-one onto the Research Strategy headings:
- **Factor 1 -- Importance of the Research**: significance and innovation assessed *together*, scored.
- **Factor 2 -- Rigor and Feasibility**: the approach, scored.
- **Factor 3 -- Expertise and Resources**: investigators and environment, rated sufficient / additional
  expertise or resources needed, rather than scored.

Fellowship, training, and career-development applications keep their own criteria, so read the FOA rather
than assuming. Editing consequence: write Significance and Innovation as one continuous argument about
importance instead of two independently defended claims, and make sure the feasibility evidence a
reviewer needs for Factor 2 sits in the Approach where they will look for it.
Reference: https://grants.nih.gov/policy-and-compliance/policy-topics/peer-review/simplifying-review/framework
